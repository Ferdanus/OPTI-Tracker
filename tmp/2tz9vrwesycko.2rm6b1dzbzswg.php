                            ...
                            <div>
                                <?php if ($order['jenis_layanan_opti'] == 'selulosa'): ?>
                                    <a href="<?= ($BASE) ?>/order/<?= ($order['id']) ?>/rancop-selulosa" class="btn btn-primary btn-sm fw-semibold shadow-sm">
                                        <i class="bi <?= (($user_id == $order['pic_proposal_id'] || $is_tim_kerja || $is_pejabat || $is_superadmin) ? 'bi-diagram-3' : 'bi-eye') ?> me-1"></i> <?= (($user_id == $order['pic_proposal_id'] || $is_tim_kerja || $is_pejabat || $is_superadmin) ? 'Skenario & Rancop' : 'Lihat Skenario & Rancop')."
" ?>
                                    </a>
                                <?php endif; ?>
                                <?php if ($order['jenis_layanan_opti'] == 'lingkungan'): ?>
                                    <a href="<?= ($BASE) ?>/order/<?= ($order['id']) ?>/biaya-lingkungan" class="btn btn-success btn-sm fw-semibold shadow-sm">
                                        <i class="bi <?= (($user_id == $order['pic_proposal_id'] || $is_tim_kerja || $is_pejabat || $is_superadmin) ? 'bi-calculator' : 'bi-eye') ?> me-1"></i> <?= (($user_id == $order['pic_proposal_id'] || $is_tim_kerja || $is_pejabat || $is_superadmin) ? 'Rincian Tarif SNI' : 'Lihat Tarif SNI')."
" ?>
                                    </a>
                                <?php endif; ?>
                            </div>
