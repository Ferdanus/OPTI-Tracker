...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
    <div class="card-header bg-white py-3 px-4 d-flex flex-wrap justify-content-between align-items-center border-bottom gap-2">
        <div class="d-flex align-items-center gap-3">
            ...
            <span class="badge bg-light text-dark border rounded-pill px-2.5 py-1 font-monospace" style="font-size: 0.72rem;"><?= (count($daftar_notifikasi_semua)) ?> Total Pesan</span>
        </div>
        <div class="btn-group btn-group-sm p-0.5 bg-light rounded-pill border" role="group">
            <button type="button" class="btn btn-white rounded-pill px-3 fw-semibold active shadow-xs" id="btnFilterSemua" onclick="filterNotif('all', this)" style="font-size: 0.75rem;">Semua</button>
            <button type="button" class="btn btn-transparent text-secondary rounded-pill px-3 fw-semibold" id="btnFilterUnread" onclick="filterNotif('unread', this)" style="font-size: 0.75rem;">Belum Dibaca</button>
        </div>
    </div>
    <div class="card-body p-0">
        <?php if (count($daftar_notifikasi_semua) > 0): ?>
            
                <div class="list-group list-group-flush" id="fullNotifList">
                    <?php foreach (($daftar_notifikasi_semua?:[]) as $item): ?>
                        <div class="list-group-item p-3.5 notif-row-item <?= ($item['is_read'] ? 'bg-white is-read' : 'bg-light bg-opacity-40 is-unread') ?> d-flex align-items-start gap-3 border-bottom transition-all">
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between align-items-start mb-1">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <?php if (!$item['is_read']): ?>
                                            <span class="badge bg-danger rounded-pill px-2 py-0.5" style="font-size: 0.65rem;">Baru</span>
                                        <?php endif; ?>
                                    </div>
                                </div>
    ...
    function filterNotif(type, btn) {
        ...
        rows.forEach(function(r) {
            if (type === 'all') {
                ...
                r.style.display = r.classList.contains('is-unread') ? '' : 'none';
            }
        });
    }
</script>