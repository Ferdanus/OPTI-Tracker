...
" ?>
    ...
    <!-- ======================================================== -->
    <!-- SEKSI 2: PROPOSAL / KALKULASI MENUNGGU APPROVAL KETUA TIM -->
    <!-- ======================================================== -->
    <div class="card border rounded-3 mb-4 bg-white shadow-xs">
        <div class="card-header bg-white py-3 px-3 px-md-4 d-flex justify-content-between align-items-center border-bottom">
            <h6 class="fw-bold text-dark m-0">
                <?php if ($filter_divisi == 'lingkungan'): ?>
                    2. Kalkulasi Tarif Pengujian Menunggu Approval
                    <?php else: ?>
                        <?php if ($filter_divisi == 'selulosa'): ?>
                            2. Proposal Teknis Menunggu Approval
                            <?php else: ?>2. Proposal &amp; Kalkulasi Tarif Menunggu Approval
                        <?php endif; ?>
                    
                <?php endif; ?>
            </h6>
            <span class="badge <?= (count($perlu_approval) > 0 ? 'bg-primary text-white' : 'bg-light text-secondary border') ?> px-2.5 py-1 fw-semibold">
                <?= (count($perlu_approval)) ?> <?= ($filter_divisi == 'lingkungan' ? 'Kalkulasi Uji' : ($filter_divisi == 'selulosa' ? 'Proposal' : 'Berkas / Kalkulasi'))."
" ?>
            </span>
        </div>
        ...
        <div class="card-body p-3 p-md-4 bg-light-subtle">
            <?php if (empty($perlu_approval)): ?>
                <div class="text-center py-4 bg-white rounded-3 border text-secondary small">
                    ...
                    <?= ($filter_divisi == 'lingkungan' ? 'Tidak ada kalkulasi tarif pengujian yang menunggu persetujuan.' : ($filter_divisi == 'selulosa' ? 'Tidak ada proposal teknis yang menunggu persetujuan.' : 'Tidak ada proposal atau kalkulasi tarif yang menunggu persetujuan.'))."
" ?>
                </div>
            <?php endif; ?>
                                        ...
                                        <div class="small text-secondary mb-3" style="font-size: 0.82rem; line-height: 1.5;">
                                            ...
                                            <div class="d-flex mb-1">
                                                <span class="text-muted" style="width: 120px; flex-shrink: 0;">
                                                    <?= ($p['jenis_layanan_opti'] == 'lingkungan' ? 'PIC Teknis Lab' : 'PIC Proposal')."
" ?>
                                                </span>
                                                <span class="text-muted me-2">:</span>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <span class="text-muted" style="width: 120px; flex-shrink: 0;">
                                                    <?= ($p['jenis_layanan_opti'] == 'lingkungan' ? 'Total Tarif Uji' : 'Estimasi Biaya')."
" ?>
                                                </span>
                                                <span class="text-muted me-2">:</span>
                                                <strong class="text-primary font-monospace">Rp <?= (number_format($p['estimasi_biaya'] ?: ($p['estimasi_total_biaya'] ?: 0), 0, ',', '.')) ?></strong>
                                                <?php if ($p['jenis_layanan_opti'] == 'lingkungan' && $p['total_parameter_lingkungan'] > 0): ?>
                                                    <span class="badge bg-light text-secondary border ms-2" style="font-size: 0.72rem;">
                                                        <i class="bi bi-check2-circle text-success me-1"></i><?= ($p['total_parameter_lingkungan']) ?> Parameter
                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                        ...
                                        <?php if ($p['jenis_layanan_opti'] == 'lingkungan'): ?>
                                                ...
                                                <?php if ($p['file_proposal']): ?>
                                                        ...
                                                        <a href="<?= ($BASE) ?>/<?= ($p['file_proposal']) ?>" target="_blank" class="btn btn-outline-danger btn-sm py-1 px-2.5 fw-medium" style="font-size: 0.78rem;">
                                                            <i class="bi bi-file-earmark-pdf me-1"></i> Berkas
                                                        </a>
                                                    
                                                    <?php else: ?>
                                                        <a href="<?= ($BASE) ?>/order/<?= ($p['id']) ?>/biaya-lingkungan" class="btn btn-outline-secondary btn-sm py-1 px-2.5 fw-medium" style="font-size: 0.78rem;" title="Lihat rincian kalkulasi parameter dan tarif">
                                                            <i class="bi bi-calculator me-1"></i> Rincian Tarif
                                                        </a>
                                                    
                                                <?php endif; ?>
                                                <a href="<?= ($BASE) ?>/order/<?= ($p['id']) ?>" class="btn btn-success btn-sm px-3 py-1 fw-semibold">
                                                    <i class="bi bi-shield-check me-1"></i> Review &amp; Setujui Tarif
                                                </a>
                                            ...
                                            <?php else: ?>
                                                <?php if ($p['file_proposal']): ?>
                                                        ...
                                                        <a href="<?= ($BASE) ?>/<?= ($p['file_proposal']) ?>" target="_blank" class="btn btn-outline-danger btn-sm py-1 px-2.5 fw-medium" style="font-size: 0.78rem;">
                                                            <i class="bi bi-file-earmark-pdf me-1"></i> File Proposal
                                                        </a>
                            ...
                            <td>
                                <?php if ($row['status_proposal_biaya'] == 'siap_penawaran'): ?>
                                    <span class="badge bg-success-subtle text-success border border-success-subtle fw-medium">
                                        <i class="bi bi-check-circle-fill me-1"></i> <?= ($row['jenis_layanan_opti'] == 'lingkungan' ? 'Tarif Disetujui' : 'Proposal Disetujui')."
" ?>
                                    </span>
                                <?php endif; ?>
                                <?php if ($row['status_proposal_biaya'] == 'draft' || !$row['status_proposal_biaya']): ?>
                                    <span class="badge bg-light text-secondary border fw-medium">
                                        <i class="bi bi-pencil-square me-1"></i> <?= ($row['jenis_layanan_opti'] == 'lingkungan' ? 'Penyusunan Tarif Lab' : 'Penyusunan Proposal')."
" ?>
                                    </span>
                                <?php endif; ?>
                            </td>
