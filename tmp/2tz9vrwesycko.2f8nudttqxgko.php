...
" ?>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-5">
                ...
                <div class="card-body p-3 p-md-4">
                    ...
                    <?php if ($surat_masuk): ?>
                        <div class="pt-3 mt-3 border-top">
                            <button type="button" class="btn btn-outline-danger btn-sm w-100 fw-semibold d-flex align-items-center justify-content-center gap-1.5 shadow-xs" onclick="openSuratMasukCanvasModal('<?= ($surat_masuk['id']) ?>')">
                                <i class="bi bi-file-earmark-pdf-fill text-danger fs-6"></i> Pratinjau Surat Permohonan Klien
                            </button>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <!-- Card 3: Jejak Audit & Riwayat Tracking -->
            <div class="card border rounded-3 bg-white shadow-xs mb-4">
                <div class="card-header bg-white py-2.5 px-3 px-md-4 border-bottom d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center gap-2">
                        <div class="rounded-circle bg-primary-subtle text-primary p-1 d-flex align-items-center justify-content-center" style="width: 26px; height: 26px;">
                            <i class="bi bi-clock-history" style="font-size: 0.8rem;"></i>
                        </div>
                        <h6 class="m-0 fw-bold text-dark font-display" style="font-size: 0.88rem;">Riwayat &amp; Audit Waktu</h6>
                    </div>
                    <span class="badge bg-light text-secondary border font-monospace" style="font-size: 0.68rem;">
                        <?= (!empty($activity_logs) ? count($activity_logs) : 0) ?> Aktivitas
                    </span>
                </div>
                        ...
                        <div class="audit-timeline">
                            <?php foreach (($activity_logs?:[]) as $log): ?>
                                <div class="audit-item">
                                    ...
                                    <div class="audit-bubble shadow-none">
                                        <div class="d-flex justify-content-between align-items-center mb-1 flex-wrap gap-1">
                                            <div class="d-flex align-items-center gap-2">
                                                <strong class="text-dark font-display" style="font-size: 0.84rem;"><?= ($log['user_nama'] ?: 'Sistem') ?></strong>
                                                <span class="badge <?= ($log['user_role'] == 'ketua_tim' ? 'bg-primary-subtle text-primary border border-primary-subtle' : 'bg-secondary-subtle text-secondary') ?> px-2 py-0.5 rounded-pill" style="font-size: 0.68rem; letter-spacing: 0.2px;">
                                                    <?= ($log['user_role'] == 'ketua_tim' ? 'Ka. Tim OPTI' : 'PIC Peneliti')."
" ?>
                                                </span>
                                            </div>
                    ...
                    <?php if (empty($activity_logs)): ?>
                        <div class="text-center py-4 text-muted">
                            <i class="bi bi-hourglass-split fs-3 opacity-50 d-block mb-1"></i>
                            <p class="small mb-0">Belum ada aktivitas tercatat.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                    ...
                    <form action="<?= ($BASE) ?>/order/<?= ($order['id']) ?>/proposal/simpan" method="POST" enctype="multipart/form-data" id="formProposal">
                        ...
                        <div class="row g-3.5 mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-light text-muted fw-bold px-3" style="font-size: 0.84rem;">Rp</span>
                                    <input type="text" id="inputBiayaDisplay" class="form-control fw-bold text-primary font-monospace py-2" style="font-size: 0.92rem;" 
                                           placeholder="0" oninput="formatRupiahInput(this)" <?= (!$can_edit ? 'disabled' : '') ?> required>
                                    <input type="hidden" name="estimasi_total_biaya" id="inputBiayaReal" value="<?= ((int)($proposal['estimasi_total_biaya'] ?: ($order['estimasi_biaya'] ?: 0))) ?>">
                                </div>
                            </div>
                        </div>
...
" ?>
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            <!-- Tombol Modal Pratinjau In-App Bebas IDM (Memuat file asli yang diunggah) -->
...
<!-- JavaScript Drag-and-Drop, Live Preview & Base64 Memory Canvas Engine -->
<script>
if (window.pdfjsLib) {
...
function loadProposalData(orderId) {
    ...
    fetch(apiUrl)
        .then(function(res) {
            if (!res.ok) throw new Error('Gagal memuat data proposal');
            return res.json();
        })
        .then(function(json) {
            if (!json.success) throw new Error(json.message || 'Data proposal gagal diproses');

            const headerEl = document.getElementById('modalPdfFilenameHeader');
            if (headerEl && json.filename) {
                headerEl.textContent = json.filename + ' • Order #' + '<?= ($order['nomor_order']) ?>';
            }

            if (json.is_pdf && json.base64) {
                const uint8Array = base64ToUint8Array(json.base64);
                return pdfjsLib.getDocument({ data: uint8Array }).promise;
            } else {
                if (spinner) spinner.style.display = 'none';
                if (nonPdfNotice) {
                    nonPdfNotice.classList.remove('d-none');
                    document.getElementById('nonPdfDocTitle').textContent = 'Berkas: ' + json.filename;
                    document.getElementById('nonPdfDocSubtitle').textContent = 'Format berkas (' + json.ext.toUpperCase() + ') tersimpan dengan aman di server.';
                }
                return null;
            }
        })
...
// ========================================================
// BASE64 JSON PDF.JS CANVAS VIEWER (SURAT MASUK KLIEN)
// ========================================================
function openSuratMasukCanvasModal(suratId) {
    const modalEl = document.getElementById('modalPreviewSuratMasukProposal');
    if (!modalEl) return;

    const bsModal = bootstrap.Modal.getOrCreateInstance(modalEl);
    bsModal.show();

    loadSuratMasukData(suratId);
}
...
function loadSuratMasukData(suratId) {
    ...
    const baseUri = '<?= ($BASE) ?>';
    const apiUrl = baseUri + '/surat-masuk/' + suratId + '/raw-data?t=' + Date.now();

    fetch(apiUrl)
        .then(function(res) {
            if (!res.ok) throw new Error('Gagal memuat data surat masuk');
            return res.json();
        })
        .then(function(json) {
            if (!json.success || !json.base64) throw new Error(json.message || 'Berkas surat tidak ditemukan');
            const uint8Array = base64ToUint8Array(json.base64);
            return pdfjsLib.getDocument({ data: uint8Array }).promise;
        })
        .then(function(pdf) {
...
// ========================================================
// FORMAT RUPIAH, TERBILANG & SHORTCUT PENAMBAH NOMINAL
// ========================================================
...
document.addEventListener('DOMContentLoaded', function() {
    const hiddenEl = document.getElementById('inputBiayaReal');
    const displayEl = document.getElementById('inputBiayaDisplay');
    if (hiddenEl && displayEl) {
