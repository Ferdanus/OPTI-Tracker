...
class SuratMasukController extends Controller {
    ...
    /**
     * Endpoint Data JSON untuk Pratinjau Surat Masuk Bebas IDM (Zero Interception)
     * Route: GET /surat-masuk/@id/raw-data
     */
    public function suratRawData() {
        $this->requireAuth();
        ...
        if (!$surat) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Data surat permohonan tidak ditemukan.']);
            exit;
        }
        ...
        if (empty($surat['file_path']) || !file_exists($filePath)) {
            ...
            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell(8, 4.8, '', 0, 0);
            $pdf->Cell(38, 4.8, 'a. Perihal Kegiatan', 0, 0);
            $pdf->Cell(4, 4.8, ':', 0, 0);
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->MultiCell(0, 4.8, $surat['perihal'], 0, 'L');
            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell(8, 4.8, '', 0, 0);
            $pdf->Cell(38, 4.8, 'b. Layanan', 0, 0);
            $pdf->Cell(4, 4.8, ':', 0, 0);
            $pdf->Cell(0, 4.8, 'Layanan Optimalisasi Teknologi Industri (OPTI)', 0, 1);
            $pdf->Cell(8, 4.8, '', 0, 0);
            $pdf->Cell(38, 4.8, 'c. Ruang Lingkup', 0, 0);
        ...
        $base64 = file_exists($filePath) ? base64_encode(file_get_contents($filePath)) : '';
